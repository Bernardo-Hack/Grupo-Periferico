...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
        const res = await fetch('http://localhost:5000/user/login', {
            ...
            // --- MUDANÇA: 'credentials' removido ---
            body: JSON.stringify({ nome, senha }),
        });

        const json = await res.json();
        
        // --- MUDANÇA: Verifica se a resposta foi 'ok' E se contém um 'token' ---
        if (res.ok && json.token) {
            // --- MUDANÇA: Salva o token no localStorage ---
            ...
            Swal.fire({
                ...
                // --- MUDANÇA: Usa o navigate para ir para o perfil ---
                navigate('/perfil');
            });
        } else {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const json = await res.json();
        if (res.ok) { // A lógica de sucesso do registro não muda
            Swal.fire({
                icon: 'success',
        ...
        <div className="login-register-container">
          ...
          <ul className="nav nav-pills nav-justified mb-3" id="ex1" role="tablist">
            <li className="nav-item" role="presentation">
              <a className="nav-link active" id="tab-login" data-mdb-pill-init href="#pills-login" role="tab" aria-controls="pills-login" aria-selected="true">
                Login
              </a>
            </li>
            <li className="nav-item" role="presentation">
              <a className="nav-link" id="tab-register" data-mdb-pill-init href="#pills-register" role="tab" aria-controls="pills-register" aria-selected="false">
                Cadastro
              </a>
            </li>
          </ul>
            ...
            <div className="tab-pane fade" id="pills-register" role="tabpanel" aria-labelledby="tab-register">
              <form className="register-form" onSubmit={handleRegister}>
                ...
                <div className="form-outline mb-4">
                  <input type="date" id="registerNascimento" className="form-control active" placeholder=" " />
